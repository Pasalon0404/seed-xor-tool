...
function encodeSeedToNumeric(seedString) {
    ...
    if (words.length !== 12 && words.length !== 24) {
        throw new Error(`Expected 12 or 24 words, but found ${words.length}. Check your spacing.`);
    }
...
document.getElementById('btn-generate').addEventListener('click', async () => {
    ...
    try {
        ...
        // 1. Generate the raw QR code logic off-screen.
        // We set margin 0 and width to 29, which gives us a pure, unscaled 29x29 matrix.
        // UX FIX: We now strictly force Version 3 (29x29) so 12-word and 24-word seeds
        // always generate the exact same grid size.
        const tempCanvas = document.createElement('canvas');
        await QRCode.toCanvas(tempCanvas, [{ data: currentNumericSeed, mode: 'numeric' }], {
            errorCorrectionLevel: 'L',
            margin: 0,
            width: 29,  // Raw 29x29 pixel matrix
            version: 3, // <-- FORCE 29x29 GRID UNIVERSALLY
            color: { dark: '#000000', light: '#ffffff' }
        });
        
        // 2. Set up our final visible canvas
        const finalCanvas = document.getElementById('qr-canvas');
        const ctx = finalCanvas.getContext('2d');
        const blockSize = 20; // How large each block is on screen
        const qrSize = 29 * blockSize; // 580px
        
        // Match Printable Grid padding for numbers
        const topPadding = 35;  
        ...
        // 3. BLOCK-BY-BLOCK DRAWING (ALIGNMENT FIX)
        // Instead of drawImage, we paint every block precisely relative to the padding.
        const tempCtx = tempCanvas.getContext('2d');
        const imgData = tempCtx.getImageData(0, 0, 29, 29).data;

        for (let y = 0; y < 29; y++) {
            for (let x = 0; x < 29; x++) {
                // imgData is RGBA, so we jump 4 bytes per pixel. [r, g, b, a]
                // If r is 0, the pixel is black.
                const rIndex = (y * 29 + x) * 4;
                const isBlack = imgData[rIndex] === 0;

                ctx.fillStyle = isBlack ? '#000000' : '#ffffff';
                // Paint the block precisely, offset by padding.
                ctx.fillRect(
        ...
        // 4. Draw the coordinate grid and numbers overlay (ALIGNMENT FIX)
        ctx.font = '14px monospace';
        ctx.fillStyle = '#888888';
        ctx.textAlign = 'center';
        ctx.textBaseline = 'middle';

        // Draw lines precisely on the module boundaries (x * 20px)
        for (let i = 0; i <= 29; i++) {
            ctx.beginPath();
            // Highlight every 5th line (Matching Printable Guide bolds)
            ctx.strokeStyle = i % 5 === 0 ? 'rgba(255, 0, 0, 0.5)' : 'rgba(255, 0, 0, 0.2)';
            ctx.lineWidth = i % 5 === 0 ? 2 : 0.8;
            
            // Vertical lines (Align with left edge + module multiplier)
            let vx = leftPadding + (i * blockSize);
            ctx.moveTo(vx, topPadding);
            ctx.lineTo(vx, topPadding + qrSize);
            
            // Horizontal lines (Align with top edge + module multiplier)
            let hy = topPadding + (i * blockSize);
            ctx.moveTo(leftPadding, hy);
            ctx.lineTo(leftPadding + qrSize, hy);
            ctx.stroke();

            // 5. Draw axis numbers (1-29) - Skipping even numbers to reduce clutter
            if (i < 29 && i % 2 === 0) {
                // Top X-axis (Centered in the module column)
                ctx.fillText((i + 1).toString(), leftPadding + (i * blockSize) + (blockSize / 2), topPadding / 2);
                // Left Y-axis (Centered in the module row)
...
document.getElementById('btn-print-blank').addEventListener('click', () => {
    // Create a beautifully structured, printable SVG document
    ...
    // 1. Draw the 3 required anchor/finder patterns FIRST using strict fills (no strokes!)
    // This prevents the borders from bleeding over the grid lines.
    const drawAnchor = (cx, cy) => {
        // Outer 7x7 Black Square
        svg += `<rect x="${cx}" y="${cy}" width="7" height="7" fill="black" />`;
        // Inner 5x5 White Square (carves out the ring)
        svg += `<rect x="${cx+1}" y="${cy+1}" width="5" height="5" fill="white" />`;
        // Innermost 3x3 Black Square (the bullseye)
    ...
    // 2. Draw the grid lines over the top
    for (let i = 0; i <= 29; i++) {
        // Make every 5th line bold to act as a guide
    ...
    // 3. Add axis numbers along the top and left
    for(let i=0; i<29; i++) {
         if (i % 2 === 0) {
    ...
    // Inject the SVG into the hidden print area and trigger the browser print dialog
...
document.getElementById('btn-camera-import').addEventListener('click', () => {
    ...
    html5QrCodeImport.start(
        ...
        (decodedText) => {
            document.getElementById('seed-input').value = decodedText;
            html5QrCodeImport.stop();
...
document.getElementById('btn-show-decrypt').addEventListener('click', () => {
    const box = document.getElementById('decrypt-box');
    box.style.display = box.style.display === 'none' ? 'block' : 'none';
});

document.getElementById('btn-process-decrypt').addEventListener('click', async () => {
    ...
    reader.onload = async (e) => {
        try {
            ...
            // UPGRADE 2.0: Grab ANY streak of 12 or more valid words.
            let foundWords = [];
            let currentStreak = [];
            ...
            if (foundWords.length > 0) {
                document.getElementById('seed-input').value = foundWords.join(" ");
                
                if (foundWords.length === 12 || foundWords.length === 24) {
                    statusBox.className = 'status success'; 
                    statusBox.innerText = `Found a perfect ${foundWords.length}-word sequence!`;
                    ...
                    statusBox.innerText = `Found ${foundWords.length} words. Please delete the extra conversational word (like "seed") from the box above.`;
                }
                statusBox.style.display = 'block';
            } else {
                statusBox.className = 'status error'; 
